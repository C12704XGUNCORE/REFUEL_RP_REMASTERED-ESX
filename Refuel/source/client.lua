...
-- Définition des clés de configuration du véhicule électrique sur les hachages.
for _, vehHash in pairs(config.electricVehicles) do
    config.electricVehicles[vehHash] = vehHash
end

-- Obtenez le carburant d'un véhicule, qui est défini sur une entité.
...
-- Réglage du carburant sur l'entité véhicule à l'aide du décor.
...
-- renvoie la position de la pompe si un joueur se trouve à proximité.
...
-- Dessine du texte 3D sur les coordonnées.
...
-- utilisé pour charger l'animation de remplissage manuellement.
...
-- Utilisé pour jouer l'effet de verser du carburant de la buse.
...
-- Placer le véhicule devant le joueur.
...
-- Créez une buse, une corde et attachez-les au joueur.
...
-- détachez la buse de tout et cachez l'interface utilisateur.
function dropNozzle()
...
-- supprimez la buse et la corde et masquez l'interface utilisateur.
        ...
        if vehicleFueling then
            ...
            if usingCan then
                ...
                if not config.standalone and cost ~= 0 then
                    TriggerServerEvent("Refuel:pay", cost)
                    cost = 0
                end
            end
        end
...
-- Saisir et ramener la buse de la pompe.
CreateThread(function()
    local wait = 500
    while true do
        Wait(wait)
        if pump then
            ...
            if not holdingNozzle and not nozzleInVehicle and not nozzleDropped then
                ...
                elseif weapon and GetSelectedPedWeapon(ped) == 883325847 and ammo < 4500 then
                    ...
                    jerryCanText = " \n$" .. price .. " Rechargé le Jerrican [G]"
                end
                DrawText3D(pump.x, pump.y, pump.z + 1.2, "Saisir la Buse [E]" .. jerryCanText)
                ...
                if IsControlJustPressed(0, 47) then
                    if not config.standalone then
                        if NDCore.Functions.GetSelectedCharacter().bank > price then
                            TriggerServerEvent("Refuel:jerryCan", price)
            ...
            elseif holdingNozzle and not nearTank and pumpHandle == usedPump then
                DrawText3D(pump.x, pump.y, pump.z + 1.2, "Reposé la Buse [E]")
                if IsControlJustPressed(0, 51) then
                    LoadAnimDict("anim@am_hold_up@male")
                    TaskPlayAnim(ped, "anim@am_hold_up@male", "shoplift_high", 2.0, 8.0, -1, 50, 0, 0, 0, 0)
                    Wait(300)
...
-- Fixation et prise de la buse du véhicule, et chute de la buse du joueur ou du véhicule.
CreateThread(function()
    local wait = 500
    while true do
        Wait(wait)
        if holdingNozzle or nozzleInVehicle or nozzleDropped then
            ...
            -- déposer la buse et la retirer si elle est éloignée de la pompe.
            if pump then
            ...
            if nozzle and pumpCoords then
                ...
                if nozzleDropped and #(nozzleLocation - pedCoords) < 1.5 then
                    DrawText3D(nozzleLocation.x, nozzleLocation.y, nozzleLocation.z, "Prendre la Buse [E]")
                    if IsControlJustPressed(0, 51) then
            ...
            -- Animations pour le ravitaillement manuel et effet pour la pulvérisation de carburant.
            ...
            -- fixer et retirer la buse du véhicule.
            if veh then
                ...
                if tankPosition and #(pedCoords - tankPosition) < 1.2 then
                    if not nozzleInVehicle and holdingNozzle then
                        nearTank = true
                        DrawText3D(tankPosition.x + textModifiedPosition.x, tankPosition.y + textModifiedPosition.y, tankPosition.z + zPos + textModifiedPosition.z, "Attacher la Buse [E]")
                        if IsControlJustPressed(0, 51) then
                            LoadAnimDict("timetable@gardener@filling_can")
                            TaskPlayAnim(ped, "timetable@gardener@filling_can", "gar_ig_5_filling_can", 2.0, 8.0, -1, 50, 0, 0, 0, 0)
                            Wait(300)
                    ...
                    elseif nozzleInVehicle then
                        DrawText3D(tankPosition.x + textModifiedPosition.x, tankPosition.y + textModifiedPosition.y, tankPosition.z + zPos + textModifiedPosition.z, "Prendre la Buse [E]")
                        if IsControlJustPressed(0, 51) then
                            LoadAnimDict("timetable@gardener@filling_can")
                            TaskPlayAnim(ped, "timetable@gardener@filling_can", "gar_ig_5_filling_can", 2.0, 8.0, -1, 50, 0, 0, 0, 0)
                            Wait(300)
...
-- faire le plein à l'aide d'un jerrican.
CreateThread(function()
    local wait = 500
    while true do
        Wait(wait)
        if GetSelectedPedWeapon(ped) == 883325847 and not holdingNozzle and not nozzleInVehicle then
            ...
            if veh then
                ...
                if tankPosition and #(pedCoords - tankPosition) < distance then
                    ...
                    DrawText3D(tankPosition.x, tankPosition.y, tankPosition.z + zPos, math.floor(fuel) .. "% Ravitailler [E]")
                    local ammo = GetAmmoInPedWeapon(ped, 883325847)
...
-- Créez des blips pour chaque emplacement de station-service.
CreateThread(function()
...
-- faire apparaître des pompes sur la carte.
CreateThread(function()
...
-- Enregistrez le décor de carburant
CreateThread(function()
